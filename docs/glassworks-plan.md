...
## Atlas budget (real constraint, flagged now)

New tiles: 3 ores + 5 powders + 6 glass (clear + 5 colors, colored
generated by tinting the clear art) + kiln mouth ×2 + quern ≈ 17.
`FIRST_FREE_SLOT` moves 216 → ~233, leaving mods ~23 slots. That's
one more feature of this size before the 256-tile atlas needs to
grow (a 32×32 atlas or a second texture layer — its own small plan;
slot numbers are painted at explicit coordinates in the procedural
art, so it's a mechanical but wide refactor). Gemini prompts for
every new tile as usual.
...
Stages 1–2 are a small standalone commit (falling sand + clear glass
+ greenhouse). Stages 3–4 are the big one. Stage 5 rides alone at
the end, cuttable without a trace.
