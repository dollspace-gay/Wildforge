...
use crate::renderer::{Renderer, FrameInput, GpuChunk};
use crate::chunk::ChunkPos;
use super::ScreenshotReadback;

impl Renderer {
    pub(in crate::renderer) fn encode_capture(&self, encoder: &mut wgpu::CommandEncoder, f: &FrameInput<'_>, visible: &[(&ChunkPos, &GpuChunk)], path: String, metadata: Option<crate::visual_capture::CaptureMetadata>) -> ScreenshotReadback {
            ...
            ScreenshotReadback { path, buf, texture, w, h, bpr, metadata, diagnostic }
    }
    pub(in crate::renderer) fn write_capture(&self, shot: ScreenshotReadback) {
        let ScreenshotReadback { path, buf, texture: _texture, w, h, bpr, metadata, diagnostic } = shot;
            let bgra = matches!(
                self.config.format,
