...
impl UiBatch {
    pub fn new() -> UiBatch {
        UiBatch { verts: Vec::with_capacity(1024) }
    }
    ...
    fn quad(&mut self, min: [f32; 2], max: [f32; 2], uv0: [f32; 2], uv1: [f32; 2], color: [f32; 4]) {
        let v = |x: f32, y: f32, u: f32, vv: f32| UiVertex { pos: [x, y], uv: [u, vv], color };
        let (a, b, c, d) = (
