...
impl Construction {
    pub(super) fn templates(&self) -> &[Template] {
        &self.templates
    }
    pub(super) fn template(&self, name: &str) -> Option<&Template> {
        self.templates.iter().find(|template| template.name == name)
    }
    pub(super) fn restore_templates(&mut self, templates: Vec<Template>) {
        self.templates = templates;
    }
    pub(super) fn insert_template(&mut self, template: Template) {
    ...
    pub(super) fn pending_at(&self, anchor: BlockPos) -> Option<&PendingFill> {
        self.pending.iter().find(|fill| fill.anchor == anchor)
    }
    ...
    pub(super) fn structures(&self) -> &[LocalStructure] {
        &self.structures
    }
    pub(super) fn structures_mut(&mut self) -> &mut [LocalStructure] {
        &mut self.structures
    }
    pub(super) fn structure(&self, id: LocalStructureId) -> Option<&LocalStructure> {
        self.structures.iter().find(|structure| structure.id == id)
    }
    pub(super) fn structure_mut(&mut self, id: LocalStructureId) -> Option<&mut LocalStructure> {
        self.structures
            .iter_mut()
    ...
    pub(super) fn take_structure(&mut self, index: usize) -> LocalStructure {
        self.structures.remove(index)
    }
    pub(super) fn return_structure(&mut self, index: usize, structure: LocalStructure) {
