...
impl ReplicaWorld {
    ...
    pub(crate) fn unload_chunk(&mut self, position: ChunkPos) {
        self.terrain.remove(&position);
    }
    ...
    pub(crate) fn falling_blocks(&self) -> &[FallingBlock] {
        &self.falling
    }
    ...
    pub(crate) fn implement_tooltip(&self, stack: ItemStack, exact: bool) -> Vec<String> {
        ...
        self.observations
            .implement(stack.arcane_id)
            .map_or_else(Vec::new, |state| state.tooltip(clean, exact))
    }

    pub(crate) fn implement_visual(
        &self,
        stack: ItemStack,
        ...
        crate::world::item_presentation::implement_visual(
            &self.registry,
            kind,
            self.observations.charge(stack.arcane_id),
        )
    }
