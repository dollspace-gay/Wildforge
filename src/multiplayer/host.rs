...
use crate::net::{
    self, C2S, HostEvent, MAX_GUEST_VIEW_DIST, MobSnap, ModerationAction, Refusal, RefusalCode,
    S2C, StackSnap, batch_snapshot,
};
...
/// does not need, and this is what keeps the snapshot bounded by perception
/// instead of by the size of the world.
...
impl HostSession {
    ...
    pub fn pump(
        ...
        // Chunk streaming toward each guest, out to the distance it asked
        // for rather than a fixed ring of five.
        ...
        //
        // Built per guest and culled to what that guest can reach. These used
        // to be one broadcast of the entire world, which meant a datagram
        // that grew with the mob cap rather than with perception — and QUIC
        // refuses an oversized datagram outright, so past roughly forty mobs
        // every guest silently stopped seeing any of them.
        ...
        if self.snapshot_timer >= 0.05 {
            ...
            if host.is_some() {
                everyone.push((0u32, host_pos, host_yaw, host_held, host_style));
            }
            ...
            for id in ids {
                ...
                // Other players stay visible past the mob horizon: a person
                // on the skyline is the point of playing together.
    ...
    fn on_join(&mut self, server: &mut Server, join: AuthenticatedJoin, fx: &mut Vec<HostFx>) {
        ...
        self.guests.insert(
            ...
            Guest {
                ...
                // Until the guest says otherwise it gets the old fixed ring,
                // which is also a sane floor for a client that never asks.
                view_dist: 5,
                chunk_requests: 0,
                chunk_window: 0.0,
                edits: 0,
    ...
    /// The chunk each guest is standing in, and the widest radius any of them
    /// was granted. What the world has to keep resident on their behalf.
    ...
    /// Put one already-batched snapshot on the wire.
